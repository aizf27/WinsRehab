...
class dt_info_Fragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //再尝试从 savedStateHandle 获取最新值（由主页写入）
        //动态覆盖
        val savedStateHandle = findNavController().previousBackStackEntry?.savedStateHandle
        savedStateHandle?.get<String>("doctorCode")?.let { doctorCode = it }
        savedStateHandle?.get<Int>("totalCount")?.let { totalCount = it }
    ...
    private fun saveDoctorInfo() {
        ...
        viewModel.saveDoctorInfo(doctor) { success ->
            if (success) {
                ...
                // 使用Fragment Result API传递更新通知
                val result = Bundle()
                result.putBoolean("doctor_info_updated", true)
                parentFragmentManager.setFragmentResult("doctor_info_result", result)
                
                findNavController().navigateUp() // 返回上一个Fragment（主页）
